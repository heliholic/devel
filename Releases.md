...
Currently there are four different kind of wiggles:
...
SBUS output is available as a tech preview, on CLI only.
It allows expanding the number of servo outputs with an expander,
or using SBUS servos directly.
...
four ESC vendors/models are supported:
