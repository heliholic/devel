...
void validateAndFixGyroConfig(void)
{
    ...
    float cutoff_limit = 0.45f * gyro.sampleRateHz / (filtDenom ? filtDenom : pidDenom);

    adjustFilterLimit(&gyroConfigMutable()->gyro_decimation_hz, LPF_MAX_HZ);
    adjustFilterLimit(&gyroConfigMutable()->gyro_lpf1_static_hz, cutoff_limit);
    adjustFilterLimit(&gyroConfigMutable()->gyro_lpf2_static_hz, cutoff_limit);
    adjustFilterLimit(&gyroConfigMutable()->gyro_soft_notch_hz_1, cutoff_limit);
    adjustFilterLimit(&gyroConfigMutable()->gyro_soft_notch_cutoff_1, 0);
    adjustFilterLimit(&gyroConfigMutable()->gyro_soft_notch_hz_2, cutoff_limit);
