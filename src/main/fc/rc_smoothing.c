...
FAST_CODE void rcSmoothingFilterUpdate(int currentRxRateUs)
{
    if (rcSmoothing.dynamicCutoffs) {
        ...
        DEBUG32_SET(DEBUG_RC_SMOOTHING_RATE, 0, currentRxRateUs);
        DEBUG32_SET(DEBUG_RC_SMOOTHING_RATE, 1, rcSmoothing.currentFrameTimeUs);
        DEBUG32_SET(DEBUG_RC_SMOOTHING_RATE, 2, rcSmoothing.averageFrameTimeUs);
        DEBUG32_SET(DEBUG_RC_SMOOTHING_RATE, 3, rcSmoothing.cutoffFreq);
    }
}
...
FAST_CODE float rcSmoothingFilterApply(int axis, float input)
{
    ...
    if (axis < 4) {
        ...
        DEBUG_AXIS_SET(DEBUG_RC_SMOOTHING, axis, 1, lrintf(input * 1000));
        DEBUG_AXIS_SET(DEBUG_RC_SMOOTHING, axis, 2, lrintf(output * 1000));
    }
