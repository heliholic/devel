...
void adcInit(const adcConfig_t *config)
{
    ...
    if (config->bec.enabled) {
        adcOperatingConfig[ADC_VBEC].tag = config->bec.ioTag;
        adcOperatingConfig[ADC_VBEC].adcDevice = config->bec.device;
    }
    if (config->bus.enabled) {
        adcOperatingConfig[ADC_VBUS].tag = config->bus.ioTag;
        adcOperatingConfig[ADC_VBUS].adcDevice = config->bus.device;
    }
    if (config->ext.enabled) {
        adcOperatingConfig[ADC_VEXT].tag = config->ext.ioTag;
        adcOperatingConfig[ADC_VEXT].adcDevice = config->ext.device;
    }
