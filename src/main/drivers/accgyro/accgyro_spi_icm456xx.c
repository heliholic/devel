...
#define ICM456XX_REG_MISC1                      0x35
...
#define ICM456XX_SIFS_I3C_STC_CFG               0x68
...
// REG_MISC1 - 0x35
#define ICM456XX_OSC_ID_OVRD_MCLK               0x00
#define ICM456XX_OSC_ID_OVRD_CLKIN              0x08
#define ICM456XX_OSC_ID_OVRD_MASK               0x0F
...
#define ICM456XX_I3C_STC_MODE                   (1 << 2)
...
static void icm456xx_enableExternalClock(const extDevice_t *dev)
{
    if (gyroExternalClockInit(dev, ICM456XX_CLKIN_FREQ)) {
        ...
        uint8_t i3c_stc = spiReadRegMsk(dev,ICM456XX_SIFS_I3C_STC_CFG);
        i3c_stc &= ~ICM456XX_I3C_STC_MODE;
        spiWriteReg(dev, ICM456XX_SIFS_I3C_STC_CFG, i3c_stc);

        // uint8_t misc1 = spiReadRegMsk(dev, ICM456XX_REG_MISC1);
        // misc1 &= ~ICM456XX_OSC_ID_OVRD_MASK;
        // misc1 |= ICM456XX_OSC_ID_OVRD_CLKIN;
        // spiWriteReg(dev, ICM456XX_REG_MISC1, misc1);
    }
}
#endif
