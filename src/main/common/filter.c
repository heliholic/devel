...
uint32_t ewma1FilterWeight(float cutoff, float sampleRate)
{
    float omega = tan_approx(M_PIf * cutoff / sampleRate);
    float weight = (1.0f / omega + 1.0f) / 2.0f;

    return lrintf(weight);
}
...
void ewma1FilterInitWeight(ewma1Filter_t *filter, uint32_t weight)
{
    filter->y1 = 0;
    filter->N = 0;
    filter->W = weight;
...
void ewma1FilterUpdateWeight(ewma1Filter_t *filter, uint32_t weight)
{
    filter->W = weight;
    filter->N = MIN(filter->N, filter->W);
}
...
uint32_t ewma2FilterWeight(float cutoff, float sampleRate)
{
    // order=2: 1 / sqrt( (2^(1 / order) - 1)) = 1.553773974
...
void ewma2FilterInitWeight(ewma2Filter_t *filter, uint32_t weight)
{
    filter->y1 = 0;
    filter->y2 = 0;
    filter->N = 0;
...
void ewma2FilterUpdateWeight(ewma2Filter_t *filter, uint32_t weight)
{
    filter->W = weight;
    filter->N = MIN(filter->N, filter->W);
}
...
uint32_t ewma3FilterWeight(float cutoff, float sampleRate)
{
    // order=3: 1 / sqrt( (2^(1 / order) - 1)) = 1.961459177
...
void ewma3FilterInitWeight(ewma3Filter_t *filter, uint32_t weight)
{
    filter->y1 = 0;
    filter->y2 = 0;
    filter->y3 = 0;
...
void ewma3FilterUpdateWeight(ewma3Filter_t *filter, uint32_t weight)
{
    filter->W = weight;
    filter->N = MIN(filter->N, filter->W);
}
...
{
    const float output = filter->b * input + filter->x1;

    filter->x1 = filter->b * input - filter->a * output;
    filter->y1 = output;
