...
float difFilterGain(float cutoff, float sampleRate)
{
    float omega = tan_approx(M_PIf * cutoff / sampleRate);
    float alpha = 2.0f / (1.0f / omega + 1.0f);
...
{
    filter->intg = 0;
    filter->gain = difFilterGain(cutoff, sampleRate);
}

void difFilterInitGain(difFilter_t *filter, float gain)
{
    filter->intg = 0;
    filter->gain = gain;
}
...
void difFilterUpdateGain(difFilter_t *filter, float gain)
{
    filter->gain = gain;
}
...
{
    float y1 = (input - filter->intg) * filter->gain;
    filter->intg += y1;
    return y1;
}
