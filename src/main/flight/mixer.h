...
static inline float mixerGetServoOutput(uint8_t i)
{
    return mixerGetOutput(MIXER_SERVO_OFFSET + i);
}

static inline float mixerGetMotorOutput(uint8_t i)
{
    return mixerGetOutput(MIXER_MOTOR_OFFSET + i);
}
...
static inline bool pidAxisSaturated(uint8_t i)
{
    return mixerSaturated(MIXER_IN_STABILIZED_ROLL + i);
}
