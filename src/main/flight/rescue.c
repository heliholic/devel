...
static void rescueSlowExit(void)
{
    for (int i=0; i<4; i++)
        rescue.setpoint[i] = getSetpoint(i);

    rescueApplyLimits(rescue.exitAccel);
}
...
float rescueApply(uint8_t axis, float setpoint)
{
    if (rescue.mode) {
        if (rescue.state != RSTATE_OFF && rescue.state != RSTATE_DONE)
            ...
            rescue.prevSetpoint[axis] = setpoint;
    }
...
void rescueInitProfile(const pidProfile_t *pidProfile)
{
    ...
    rescue.exitAccel = (10.0f / pidProfile->rescue.exit_time) * pidGetDT();
}
