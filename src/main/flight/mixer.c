...
static void mixerUpdateBasic(void)
{
    if (mixerConfig()->swash_type)
    {
        ...
        const float S0 = mixer.swashTrim[0];
        const float S1 = mixer.swashTrim[1];
        const float S2 = mixer.swashTrim[2];

        switch (mixerConfig()->swash_type) {
            case SWASH_TYPE_120:
                mixerSetServoOutput(0, 0.5f * SC - SP + S0);
                mixerSetServoOutput(1, 0.5f * SC + 0.86602540f * SR + 0.5f * SP + S1);
                mixerSetServoOutput(2, 0.5f * SC - 0.86602540f * SR + 0.5f * SP + S2);
                break;

            case SWASH_TYPE_135:
                mixerSetServoOutput(0, 0.5f * SC - SP + S0);
                mixerSetServoOutput(1, 0.5f * SC + 0.70710678f * SR + 0.70710678f * SP + S1);
                mixerSetServoOutput(2, 0.5f * SC - 0.70710678f * SR + 0.70710678f * SP + S2);
                break;

            case SWASH_TYPE_140:
                mixerSetServoOutput(0, 0.5f * SC - SP + S0);
                mixerSetServoOutput(1, 0.5f * SC + 0.64278760f * SR + 0.7660444f * SP + S1);
                mixerSetServoOutput(2, 0.5f * SC - 0.64278760f * SR + 0.7660444f * SP + S2);
                break;

            case SWASH_TYPE_90L:
                mixerSetServoOutput(0, -SP + S0);
                mixerSetServoOutput(1, -SR + S1);
                break;

            case SWASH_TYPE_90V:
                mixerSetServoOutput(0,  0.70710678f * SR - 0.70710678f * SP + S0);
                mixerSetServoOutput(1, -0.70710678f * SR - 0.70710678f * SP + S1);
                break;
