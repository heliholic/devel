...
static void updateAirborneGyro(float *gyroAvg)
{
    ...
    const float gyroStillProb = (gyroMagnitude > GYRO_STILL_LIMIT) ? 1 : 0;
...
{
    accMagnitude = fabsf(acc.dev.acc_1G_rec *
        sqrtf(sq(accAvg[X]) + sq(accAvg[Y]) + sq(accAvg[Z])) - 1.0f);
    DEBUG(AIRBORNE, 2, accMagnitude * 100);

    const float accStillProb = (accMagnitude > ACC_STILL_LIMIT) ? 1 : 0;
...
//  printf("[imu]deltaT = %u, imuDeltaT = %u, currentTimeUs = %u, micros64_real = %lu\n", deltaT, imuDeltaT, currentTimeUs, micros64_real());
    ...
    airborneStatus = ARMING_FLAG(ARMED) && isSpooledUp() &&
        (gyroStillEstimate > 0.20f || accStillEstimate > 0.20f || getMaxCollective() > 0.20f);
