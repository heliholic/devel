...
void INIT_CODE pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    pid.errorDecayRateFFI    = (pidProfile->error_decay_time_ffi)    ? (10 * pid.dT / pidProfile->error_decay_time_ffi) : 0;
    ...
    pid.errorDecayLimitFFI    = pid.dT * (pidProfile->error_decay_limit_ffi ? pidProfile->error_decay_limit_ffi : 1e6);
...
static inline void rotateAxisError(void)
{
    if (pid.errorRotation) {
        ...
        const float fx = pid.data[PID_ROLL].ffError;
        const float fy = pid.data[PID_PITCH].ffError;

        pid.data[PID_ROLL].ffError  -= fx * C - fy * S;
        pid.data[PID_PITCH].ffError -= fy * C + fx * S;
    }
}
...
/** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
 **
 ** MODE 3 - Test mode for FF I-term
 **
 **   gyroFilter => Kp => P-term
 **   gyroFilter => difFilter => Kd => D-term
 **   gyroFilter => Relax => Ki => I-term
...
static void pidApplyCyclicMode3(uint8_t axis)
{
  ...
  //// FF I-term
    ...
    // FF change modulated by collective
    const float collMod = copysignf(POWER2(pid.collective), pid.collective);
    const float ffDelta = saturation ? 0 : errorRate * pid.dT * collMod;

    // Calculate J-component
    pid.data[axis].ffError = limitf(pid.data[axis].ffError + ffDelta, pid.errorLimit[axis]);
    pid.data[axis].J = pid.coef[axis].Kj * pid.data[axis].ffError * pid.collective;

    // Apply error decay
    pid.data[axis].ffError -= isAirborne() ?
      limitf(pid.data[axis].ffError * pid.errorDecayRateFFI, pid.errorDecayLimitFFI):
      pid.data[axis].ffError * pid.errorDecayRateGround;
  ...
  //// PID Sum
    ...
    // Calculate PID sum
    pid.data[axis].pidSum = pid.data[axis].P + pid.data[axis].I + pid.data[axis].D + pid.data[axis].F + pid.data[axis].J;
}
