...
void INIT_CODE pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    for (int i = 0; i < XYZ_AXIS_COUNT; i++) {
        ...
        lowpassFilterInit(&pid.errorFilter[i], LPF_PT1, pidProfile->error_cutoff[i], pid.freq, 0);
        lowpassFilterInit(&pid.ftermFilter[i], LPF_PT1, pidProfile->fterm_cutoff[i], pid.freq, 0);
...
static void pidApplyCyclicMode9(const pidProfile_t *pidProfile, uint8_t axis)
{
  ...
  //// D-term
    ...
    float dTerm = difFilterApply(&pid.dtermFilter[axis], dError);
...
static void pidApplyYawMode9(const pidProfile_t *pidProfile)
{
    ...
    // Get filtered gyro rate
    float gyroRate = filterApply(&pid.gyrorFilter[axis], gyro.gyroADCf[axis]);

    // Calculate error rate
    float errorRate = filterApply(&pid.errorFilter[axis], setpoint - gyroRate);
