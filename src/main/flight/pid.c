...
void INIT_CODE pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    pid.coef[PID_ROLL].Kb = ROLL_D_TERM_SCALE * pidProfile->pid[PID_ROLL].B;
    ...
    pid.coef[PID_PITCH].Kb = PITCH_D_TERM_SCALE * pidProfile->pid[PID_PITCH].B;
    ...
    pid.coef[PID_YAW].Kb = YAW_D_TERM_SCALE * pidProfile->pid[PID_YAW].B;
    ...
    pid.coef[PID_WAY].Kb = YAW_D_TERM_SCALE * pidProfile->pid[PID_WAY].B;
    ...
    pid.cyclicCrosstalkGain = pidProfile->cyclic_crosstalk_gain * ROLL_D_TERM_SCALE;

    // Pitch derivative filter
    difFilterInit(&pid.pitchDFilter, pidProfile->cyclic_crosstalk_cutoff, pid.freq);
...
static void pidApplyCyclicCrosstalk(void)
{
    ...
    const float pitchDeriv = difFilterApply(&pid.pitchDFilter, pitchRate);
...
static float linearf(float x, const uint8_t * table, int count)
{
  /* Max x in range 0..1 to piecewise linear table of size count */
  const int index = constrain(x * count, 0, count - 1);

  const int a = table[index + 0];
  const int b = table[index + 1];

  const float dy = b - a;
  const float dx = x * count - index;
...
static void pidApplyCyclicMode3(uint8_t axis, const pidProfile_t * pidProfile)
{
  ...
  //// I-term
    ...
    if (true /*isAirborne()*/) {
      decayTime = linearf(fabsf(pid.collective), pidProfile->error_decay_time_curve_cyclic, 10);
      decayRate = (decayTime > 0) ? 10 / decayTime : 0;
      decayLimit = linearf(fabsf(pid.collective), pidProfile->error_decay_limit_curve_cyclic, 10);
  ...
  //// Offset term
    ...
    // Apply error decay
    if (true /*isSpooledUp()*/) {
      decayTime = linearf(fabsf(pid.collective), pidProfile->error_decay_time_curve_offset, 10);
      decayRate = (decayTime > 0) ? 10 / decayTime : 0;
      decayLimit = linearf(fabsf(pid.collective), pidProfile->error_decay_limit_curve_offset, 10);
