...
void INIT_CODE pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    pid.coef[PID_ROLL].Ko = ROLL_O_TERM_SCALE * pidProfile->pid[PID_ROLL].O;
    ...
    pid.coef[PID_PITCH].Ko = PITCH_O_TERM_SCALE * pidProfile->pid[PID_PITCH].O;
...
static inline void rotateAxisError(void)
{
    if (pid.errorRotation) {
        ...
        const float fx = pid.data[PID_ROLL].offset;
        const float fy = pid.data[PID_PITCH].offset;

        pid.data[PID_ROLL].offset  -= fx * C - fy * S;
        pid.data[PID_PITCH].offset -= fy * C + fx * S;
    }
}
...
static void pidApplyCyclicMode3(uint8_t axis)
{
  ...
  //// Offset term
    ...
    const float offDelta = saturation ? 0 : errorRate * pid.dT * collMod;

    // Calculate J-component
    pid.data[axis].offset = limitf(pid.data[axis].offset + offDelta, pid.errorLimit[axis]);
    pid.data[axis].O = pid.coef[axis].Ko * pid.data[axis].offset * pid.collective;

    // Apply error decay
    pid.data[axis].offset -= isAirborne() ?
      limitf(pid.data[axis].offset * pid.errorDecayRateOffset, pid.errorDecayLimitOffset):
      pid.data[axis].offset * pid.errorDecayRateGround;
