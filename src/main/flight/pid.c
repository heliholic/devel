...
static void pidApplyOffsetBleed(const pidProfile_t * pidProfile)
{
    ...
    // Bleed variables
    float bleedTime = linearf(An, pidProfile->error_bleed_time_curve_offset, LOOKUP_CURVE_POINTS);
    float bleedRate = (bleedTime > 0) ? 10 / bleedTime : 0;
    float bleedLimit = linearf(An, pidProfile->error_bleed_limit_curve_offset, LOOKUP_CURVE_POINTS);
...
static void pidApplyCyclicMode3(uint8_t axis, const pidProfile_t * pidProfile)
{
  ...
  //// I-term
    ...
    if (true /*isAirborne()*/) {
      decayTime = linearf(collectiveAbs, pidProfile->error_decay_time_curve_cyclic, LOOKUP_CURVE_POINTS);
      decayRate = (decayTime > 0) ? 10 / decayTime : 0;
      decayLimit = linearf(collectiveAbs, pidProfile->error_decay_limit_curve_cyclic, LOOKUP_CURVE_POINTS);
  ...
  //// Offset term
    ...
    const float offMod = copysignf(linearf(collectiveAbs, pidProfile->offset_charge_curve, LOOKUP_CURVE_POINTS), collective) / 100.0f;
    ...
    // Apply offset decay
    if (true /*isSpooledUp()*/) {
      decayTime = linearf(collectiveAbs, pidProfile->error_decay_time_curve_offset, LOOKUP_CURVE_POINTS);
      decayRate = (decayTime > 0) ? 10 / decayTime : 0;
      decayLimit = linearf(collectiveAbs, pidProfile->error_decay_limit_curve_offset, LOOKUP_CURVE_POINTS);
