...
void INIT_CODE pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    pid.cyclicCrossCouplingGain[FD_PITCH] = pidProfile->cyclic_cross_coupling_gain * mixerRotationSign() * -CYCLIC_CROSS_COUPLING_SCALE;
...
static void pidApplyCyclicCrossCoupling(void)
{
    ...
    const float pitchComp  = rollDeriv * pid.cyclicCrossCouplingGain[FD_PITCH];
    const float rollComp   = pitchDeriv * pid.cyclicCrossCouplingGain[FD_ROLL];
