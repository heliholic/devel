...
typedef void (*govCtrlFn)(float rate, float min, float max);
...
static void govThrottleSlewControl(float rate, float min, float max)
{
    // Throttle value
    float input = gov.throttleInput;
    float output = gov.throttleOutput;

    // Loose limits while converging
    if (input < output && output > max)
        max = output;
    if (input > output && output < min)
        min = output;

    // Limit rate
    output = slewLimit(output, input, rate);

    // Limit value range
    output = constrainf(output, min, max);

    // Set governor output throttle
    gov.throttleOutput = output;
...
static void govHeadspeedSpoolUpControl(float rate, float min, float max)
{
    // PID limits
    gov.P = constrainf(gov.P, -gov.lp, gov.LP);
...
static void govFallbackControl(float __unused rate, float min, float max)
{
    // I-term is fixed
