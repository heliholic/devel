...
static float govGetBypassThrottle(const float *throttleCurve, float minThrottle, float maxThrottle)
{
...
static void govThrottleSlewControl(float minThrottle, float maxThrottle, float upRate, float downRate)
{
    if (gov.throttleInputOff) {
        ...
        // Limit change rate
        gov.throttleOutput = slewUpDownLimit(gov.throttleSlew, throttle, upRate, downRate);
    ...
    // Remember this value
    gov.throttleSlew = gov.throttleOutput;
}
...
static void govDirectActiveControl(float minThrottle, float maxThrottle, float upRate, float downRate)
{
    ...
    // Limit change rate
    throttle = slewUpDownLimit(gov.throttleSlew, throttle, upRate, downRate);

    // Remember this value
    gov.throttleSlew = throttle;

    // TTA effect
    if (gov.useTorqueAssist) {
        throttle = fminf(throttle + throttle * gov.ttaAdd, maxThrottle);
    }
