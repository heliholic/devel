...
// RPM glitch ratio
#define GOV_HS_GLITCH_DELTA             0.20f
...
static void govUpdateInputs(void)
{
    ...
    bool rpmError = ((gov.fullHeadSpeedRatio < GOV_HS_INVALID_RATIO || gov.motorRPM < 1) && gov.throttle > GOV_HS_INVALID_THROTTLE);

    // Detect RPM glitches
    bool rpmGlitch = (gov.motorRPM - filteredRPM > gov.motorRPMGlitchDelta || gov.motorRPM > gov.motorRPMGlitchLimit);

    // Error cases
    gov.motorRPMError = rpmGlitch || rpmError;
