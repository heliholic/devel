...
typedef struct {
    ...
    // Governor temporary disable
    bool            govDisabled;

    // Governor features
    bool            useDirectThrottle;
...
typedef float (*govCtrlFn)(float rate, float min, float max);
...
{
    return IS_RC_MODE_ACTIVE(BOXGOVERNORDISABLE) || gov.useDirectThrottle;
}
...
static float precompCurve(float angle, govCurve_e curve)
{
    float drag;
...
static float govHeadspeedSpoolUpControl(float rate, float min, float max)
{
    ...
    if (!((output > max && gov.C > 0) || (output < min && gov.C < 0)))
        gov.I += gov.C;
    ...
    if (!ARMING_FLAG(ARMED)) {
        ...
        switch (gov.state)
        {
            ...
            case GOV_STATE_THROTTLE_CUT:
                ...
                else if (govStateTime() > gov.throttleCutTimeout)
                    govChangeState(GOV_STATE_THROTTLE_OFF);
                break;
        ...
        switch (gov.state)
        {
            ...
            case GOV_STATE_THROTTLE_CUT:
                if (gov.throttleInput > gov.handoverThrottle && gov.motorRPMPresent)
                    govEnterRecoveryState();
                else if (!gov.motorRPMPresent || govStateTime() > gov.throttleCutTimeout) {
                    if (gov.throttleInputOff)
                        govChangeState(GOV_STATE_THROTTLE_OFF);
                    else
                        govChangeState(GOV_STATE_THROTTLE_IDLE);
...
void INIT_CODE governorInitProfile(const pidProfile_t *pidProfile)
{
    if (gov.govMode)
    {
        gov.useDirectThrottle = (pidProfile->governor.flags & BIT(GOV_FLAG_DIRECT_THROTTLE));
        gov.useDynMinThrottle = (pidProfile->governor.flags & BIT(GOV_FLAG_DYN_MIN_THROTTLE)) && !gov.useDirectThrottle;
        gov.useThreePosThrottle = (pidProfile->governor.flags & BIT(GOV_FLAG_3POS_THROTTLE)) && !gov.useDirectThrottle;
        gov.useHsAdjustment = (pidProfile->governor.flags & BIT(GOV_FLAG_HS_ON_THROTTLE)) && !gov.useThreePosThrottle && !gov.useDirectThrottle;
        gov.usePidSpoolup = (pidProfile->governor.flags & BIT(GOV_FLAG_PID_SPOOLUP)) && (gov.govMode == GOV_MODE_ELECTRIC) && !gov.useDirectThrottle;
        gov.useVoltageComp = (pidProfile->governor.flags & BIT(GOV_FLAG_VOLTAGE_COMP)) && (gov.govMode == GOV_MODE_ELECTRIC) && isBatteryVoltageConfigured() && !gov.useDirectThrottle;
        gov.usePrecomp = (pidProfile->governor.flags & BIT(GOV_FLAG_PRECOMP)) && !gov.useDirectThrottle;
        gov.useDirectPrecomp = (pidProfile->governor.flags & BIT(GOV_FLAG_DIRECT_PRECOMP)) && gov.usePrecomp && !gov.useHsAdjustment && !gov.useDirectThrottle;
        gov.useAutoRotation = (pidProfile->governor.flags & BIT(GOV_FLAG_AUTOROTATION)) && !gov.useDirectThrottle;
        ...
        gov.lp = pidProfile->governor.p_limits[1] / 100.0f;
        ...
        if (gov.usePidSpoolup) {
            ...
            govSpoolupControl = govThrottleSlewControl;
        }
    ...
    if (getMotorCount() > 0)
    {
        ...
        switch (gov.govMode)
        {
            case GOV_MODE_EXTERNAL:
                govStateUpdate = govUpdateExternalState;
                break;
        ...
        if (gov.govMode)
        {
            ...
            gov.throttleCutTimeout  = governorConfig()->gov_throttle_cut_timeout * 100;
