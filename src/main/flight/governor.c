...
static float govCalcFeedforward(void)
{
    ...
    if (gov.useTxThrottleCurve) {
        ...
        // Total feedforward
        totalFF = gov.baseThrottle + gov.K * gov.Kf * (collectiveFF + cyclicFF + yawFF);
    }
...
static void govBaseThrottleUpdate(void)
{
    if (gov.useDynBaseThrottle) {
        ...
        DEBUG(GOV_MOTOR, 4, gov.baseThrottle * 1000);
    }
}
...
static void govFallbackControl(float minThrottle, float maxThrottle, float maxRate)
{
    ...
    // Fallback "PID sum"
    gov.pidSum = gov.F * gov.fallbackThrottleRatio;
...
void INIT_CODE governorInitProfile(const pidProfile_t *pidProfile)
{
    if (gov.govMode)
    {
        ...
        gov.useTxThrottleCurve = (pidProfile->governor.flags & BIT(GOV_FLAG_TX_THROTTLE_CURVE)) && !gov.useThreePosThrottle && !gov.usePassthrough;
        gov.useHsAdjustment = (pidProfile->governor.flags & BIT(GOV_FLAG_HS_ADJUSTMENT)) && !gov.useTxThrottleCurve && !gov.useThreePosThrottle && !gov.usePassthrough;
        gov.useFallbackPrecomp = (pidProfile->governor.flags & BIT(GOV_FLAG_FALLBACK_PRECOMP)) && !gov.usePassthrough;
        gov.usePidSpoolup = (pidProfile->governor.flags & BIT(GOV_FLAG_PID_SPOOLUP)) && !gov.useTxThrottleCurve && !gov.usePassthrough;
        ...
        if (pidProfile->governor.tta_gain && !gov.usePassthrough) {
        ...
        gov.fallbackThrottleRatio = (100 - constrain(pidProfile->governor.fallback_drop, 0, 50)) / 100.0f;
