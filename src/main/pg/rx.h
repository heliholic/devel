...
typedef struct rxConfig_s {
    ...
    uint16_t rx_pwm_pulse_min;                 // Absolute minimum pulse accepted
    uint16_t rx_pwm_pulse_max;                 // Absolute maximum pulse accepted
...
typedef struct rcControlsConfig_s {
    uint16_t rc_center;                 // Stick center. Usuallay 1500 or 1520, depending on the RC system
    uint16_t rc_scale;                  // Max stick declection in us
    uint16_t rc_min_throttle;           // Throttle channel value for 0%
    uint16_t rc_max_throttle;           // Throttle channel value for 100%
    uint8_t  rc_smoothness;             // Smoothing level
    uint8_t  rc_threshold[4];           // Threshold for stick activity
    uint8_t  deadband;                  // A deadband around the stick center for pitch and roll axis
    uint8_t  yaw_deadband;              // A deadband around the stick center for yaw axis
} rcControlsConfig_t;
