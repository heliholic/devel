...
{
    return (crsfTelemetryMode != TELEMETRY_OFF);
}
...
{
    TLM_SENSOR(FLIGHT_MODE,     LEGACY_FLIGHT_MODE,   200,  200,  Nil),
    TLM_SENSOR(BATTERY,         LEGACY_BATTERY,       200,  200,  Nil),
    TLM_SENSOR(ATTITUDE,        LEGACY_ATTITUDE,      200,  200,  Nil),
    TLM_SENSOR(ALTITUDE,        LEGACY_ALTITUDE,      200,  200,  Nil),
    TLM_SENSOR(GPS,             LEGACY_GPS,           200,  200,  Nil),
};
...
{
    if (crsfTelemetryMode == TELEMETRY_NATIVE)
    {
...
{
    if (crsfTelemetryMode == TELEMETRY_CUSTOM)
    {
...
{
    if (crsfTelemetryMode == TELEMETRY_POPULATE)
    {
        static int slot = -10;
        ...
        crsfTelemetryMode = TELEMETRY_CUSTOM;
    }
...
{
    if (crsfTelemetryMode == TELEMETRY_OFF)
        return;
...
{
    crsfTelemetryMode = !crsfRxIsActive() ? TELEMETRY_OFF :
        (telemetryConfig()->crsf_telemetry_mode == CRSF_TELEMETRY_MODE_NATIVE ? TELEMETRY_NATIVE : TELEMETRY_POPULATE);
