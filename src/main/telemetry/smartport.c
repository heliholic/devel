...
static telemetrySensor_t smartportTelemetrySensors[] =
{
    ...
    TLM_SENSOR(BATTERY_VOLTAGE,         0x0210,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BATTERY_CURRENT,         0x0200,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BATTERY_CONSUMPTION,     0x5250,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BATTERY_CHARGE_LEVEL,    0x0600,   200,  3000,   1,  10,   INT),

    //TLM_SENSOR(BATTERY_CELL_COUNT,      0x????,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BATTERY_CELL_VOLTAGE,    0x0910,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(BATTERY_CELL_VOLTAGES,   0x0300,   200,  3000,   1,  10,   Cells),

    //TLM_SENSOR(PITCH_CONTROL,           0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ROLL_CONTROL,            0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(YAW_CONTROL,             0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(COLLECTIVE_CONTROL,      0x????,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(THROTTLE_CONTROL,        0x5440,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(ESC1_VOLTAGE,            0x0218,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC1_CURRENT,            0x0208,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_CAPACITY,           0x????,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC1_ERPM,               0x0508,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_POWER,              0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_THROTTLE,           0x????,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC1_TEMP1,              0x0418,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_TEMP2,              0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_BEC_VOLTAGE,        0x????,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC1_BEC_CURRENT,        0x????,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(ESC2_VOLTAGE,            0x0219,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC2_CURRENT,            0x0209,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(ESC2_CAPACITY,           0x????,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC2_ERPM,               0x0509,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC2_TEMP1,              0x0419,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(MCU_VOLTAGE,             0x0900,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BEC_VOLTAGE,             0x0901,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BUS_VOLTAGE,             0x0902,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(ESC_VOLTAGE,             0x0211,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ESC_CURRENT,             0x0201,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(BEC_CURRENT,             0x0202,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(MCU_TEMP,                0x0400,   500,  3000,   1,   2,   INT),
    TLM_SENSOR(ESC_TEMP,                0x0401,   500,  3000,   1,   2,   INT),
    TLM_SENSOR(BEC_TEMP,                0x0402,   500,  3000,   1,   2,   INT),

    TLM_SENSOR(HEADING,                 0x0840,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ALTITUDE,                0x0100,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(VARIOMETER,              0x0110,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(HEADSPEED,               0x0500,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(TAILSPEED,               0x0501,   200,  3000,   1,  10,   INT),

    TLM_SENSOR(ATTITUDE,                0x0730,   200,  3000,   1,  10,   Attitude),

    TLM_SENSOR(ACCEL_X,                 0x0700,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ACCEL_Y,                 0x0710,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(ACCEL_Z,                 0x0720,   200,  3000,   1,  10,   INT),

    //TLM_SENSOR(GPS_SATS,                0x????,   500,  3000,   1,  10,   INT),
    //TLM_SENSOR(GPS_PDOP,                0x????,   500,  3000,   1,  10,   INT),
    //TLM_SENSOR(GPS_HDOP,                0x????,   500,  3000,   1,  10,   INT),
    //TLM_SENSOR(GPS_VDOP,                0x????,   500,  3000,   1,  10,   INT),
    TLM_SENSOR(GPS_COORD,               0x0800,   200,  3000,   1,  10,   Lat),
    TLM_SENSOR(GPS_COORD,               0x0800,   200,  3000,   1,  10,   Lon),
    TLM_SENSOR(GPS_ALTITUDE,            0x0820,   200,  3000,   1,  10,   INT),
    TLM_SENSOR(GPS_HEADING,             0x0840,   200,  3000,   1,  10,   Heading),
    TLM_SENSOR(GPS_GROUNDSPEED,         0x0830,   200,  3000,   1,  10,   Knots),
    //TLM_SENSOR(GPS_HOME_DISTANCE,       0x0420,   200,  3000,   1,  10,   INT),
    //TLM_SENSOR(GPS_HOME_DIRECTION,      0x????,   200,  3000,   1,  10,   INT),

    //TLM_SENSOR(CPU_LOAD,                0x????,   500,  3000,   1,  10,   INT),
    //TLM_SENSOR(SYS_LOAD,                0x????,   500,  3000,   1,  10,   INT),
    //TLM_SENSOR(RT_LOAD,                 0x????,   500,  3000,   1,  10,   INT),

    TLM_SENSOR(MODEL_ID,                0x5460,   200,  3000,   0,   0,   INT),
    TLM_SENSOR(FLIGHT_MODE,             0x5461,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(ARMING_FLAGS,            0x5462,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(ARMING_DISABLE_FLAGS,    0x5463,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(RESCUE_STATE,            0x5454,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(GOVERNOR_STATE,          0x5465,   200,  3000,   1,   1,   INT),

    TLM_SENSOR(PID_PROFILE,             0x5471,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(RATES_PROFILE,           0x5472,   200,  3000,   1,   1,   INT),
    TLM_SENSOR(LED_PROFILE,             0x5473,   200,  3000,   1,   1,   INT),
...
static void INIT_CODE initSmartPortSensors(void)
{
    ...
    telemetryScheduleInit(smartportTelemetrySensors, ARRAYLEN(smartportTelemetrySensors), true);
