...
static bool blackboxIsLoggingEnabled(void)
{
    return (blackboxConfig()->device && (
        (blackboxConfig()->mode == BLACKBOX_MODE_BOX && IS_RC_MODE_ACTIVE(BOXBLACKBOX)) ||
        (blackboxConfig()->mode == BLACKBOX_MODE_ARM && ARMING_FLAG(ARMED)) ||
        (blackboxConfig()->mode == BLACKBOX_MODE_ARM_BOX && ARMING_FLAG(ARMED) && IS_RC_MODE_ACTIVE(BOXBLACKBOX))));
}

static bool blackboxIsLoggingPaused(void)
{
    return (blackboxConfig()->mode == BLACKBOX_MODE_ARM_BOX && !IS_RC_MODE_ACTIVE(BOXBLACKBOX));
}
