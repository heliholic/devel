...
static const blackboxDeltaFieldDefinition_t blackboxMainFields[] =
{
    ...
    {"Tbec",       -1, UNSIGNED, .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),    .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB),  CONDITION(TBEC)},
    {"Tesc2",      -1, UNSIGNED, .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),    .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB),  CONDITION(TESC2)},
...
static void writeIntraframe(void)
{
    ...
    if (testBlackboxCondition(CONDITION(BEC_TELEM))) {
        blackboxWriteSignedVB(blackboxCurrent->bec_voltage);
        blackboxWriteSignedVB(blackboxCurrent->bec_current);
    }
    if (testBlackboxCondition(CONDITION(ESC2_TELEM))) {
        blackboxWriteSignedVB(blackboxCurrent->esc2_voltage);
        blackboxWriteSignedVB(blackboxCurrent->esc2_current);
        blackboxWriteSignedVB(blackboxCurrent->esc2_capa);
        blackboxWriteSignedVB(blackboxCurrent->esc2_rpm);
    }
...
static void writeInterframe(void)
{
    ...
    if (testBlackboxCondition(CONDITION(ESC2_TELEM))) {
        blackboxWriteSignedVB((int32_t) blackboxCurrent->esc2_voltage - blackboxPrev->esc_voltage);
        blackboxWriteSignedVB((int32_t) blackboxCurrent->esc2_current - blackboxPrev->esc_current);
        blackboxWriteSignedVB((int32_t) blackboxCurrent->esc2_capa - blackboxPrev->esc_capa);
        blackboxWriteSignedVB((int32_t) blackboxCurrent->esc2_rpm - blackboxPrev->esc_rpm);
    }
    ...
    if (testBlackboxCondition(CONDITION(TBEC))) {
        blackboxWriteSignedVB((int32_t) blackboxCurrent->bec_temp - blackboxPrev->bec_temp);
    }
    if (testBlackboxCondition(CONDITION(TESC2))) {
        blackboxWriteSignedVB((int32_t) blackboxCurrent->esc_temp - blackboxPrev->esc2_temp);
    }
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    if (escData && escData->age <= ESC_BATTERY_AGE_MAX) {
        ...
        blackboxCurrent->bec_temp = escData->temperature2;
    ...
    if (escData && escData->age <= ESC_BATTERY_AGE_MAX) {
        ...
        blackboxCurrent->esc2_temp = escData->temperature;
    }
    else {
