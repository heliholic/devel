...
static bool blackboxWriteSysinfo(void)
{
    ...
    switch (xmitState.headerIndex) {
        ...
        BLACKBOX_PRINT_HEADER_LINE_CUSTOM(
            ...
            blackboxPrintfHeaderLine("gyro_rrpm_filter_bank_rpm_source", buf);
        );
        BLACKBOX_PRINT_HEADER_LINE_CUSTOM(
            char *ptr = buf;
            for (int i=0; i<RPM_FILTER_BANK_COUNT; i++) {
        ...
        BLACKBOX_PRINT_HEADER_LINE_CUSTOM(
            ...
            for (int i=0; i<RPM_FILTER_BANK_COUNT; i++) {
                if (i > 0)
                    *ptr++ = ',';
                ptr += tfp_sprintf(ptr, "%d", rpmFilterConfig()->filter_bank_rpm_min[i]);
            }
            blackboxPrintfHeaderLine("gyro_rpm_filter_bank_rpm_min", buf);
        );
        BLACKBOX_PRINT_HEADER_LINE_CUSTOM(
            char *ptr = buf;
            for (int i=0; i<RPM_FILTER_BANK_COUNT; i++) {
