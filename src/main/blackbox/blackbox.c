...
static void writeIntraframe(void)
{
    ...
    if (isFieldEnabled(FIELD_SELECT(RPM))) {
        blackboxWriteSignedVBArray(blackboxCurrent->rpm, motorCount);
    }
...
static void writeInterframe(void)
{
     ...
     */
    if (testBlackboxCondition(CONDITION(COMMAND))) {
        CALC_DELTAS(deltas, blackboxCurrent->command, blackboxLast->command, 4);
        blackboxWriteTag8_4S16(deltas);
    }

    if (testBlackboxCondition(CONDITION(SETPOINT))) {
        CALC_DELTAS(deltas, blackboxCurrent->setpoint, blackboxLast->setpoint, 4);
        blackboxWriteTag8_4S16(deltas);
    }

    if (testBlackboxCondition(CONDITION(PID))) {
        CALC_DELTAS(deltas, blackboxCurrent->axisPID_P, blackboxLast->axisPID_P, XYZ_AXIS_COUNT);
        blackboxWriteTag2_3S32(deltas);

        CALC_DELTAS(deltas, blackboxCurrent->axisPID_I, blackboxLast->axisPID_I, XYZ_AXIS_COUNT);
        blackboxWriteTag2_3S32(deltas);

        CALC_DELTAS(deltas, blackboxCurrent->axisPID_D, blackboxLast->axisPID_D, XYZ_AXIS_COUNT);
        blackboxWriteTag2_3S32(deltas);

        CALC_DELTAS(deltas, blackboxCurrent->axisPID_F, blackboxLast->axisPID_F, XYZ_AXIS_COUNT);
    ...
    if (isFieldEnabled(FIELD_SELECT(RPM))) {
        CALC_DELTAS(deltas, blackboxCurrent->rpm, blackboxLast->rpm, motorCount);
    ...
    if (testBlackboxCondition(CONDITION(DEBUG))) {
        CALC_DELTAS(deltas, blackboxCurrent->debug, blackboxLast->debug, DEBUG_VALUE_COUNT);
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    blackboxCurrent->setpoint[3] = lrintf(0); // FIXME collective
