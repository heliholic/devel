...
void blackboxInit(void)
{
    ...
    uint32_t Imul = (32 * gyro.sampleRateHz) / (1000 * activePidLoopDenom * blackboxPInterval);
    ...
    blackboxSInterval = 5 * gyro.sampleRateHz / (activePidLoopDenom * blackboxPInterval);

    // GPS frame is written at least every 10s
    blackboxGInterval = 10 * gyro.sampleRateHz / (activePidLoopDenom * blackboxIInterval);
