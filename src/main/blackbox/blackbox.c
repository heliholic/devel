...
static const blackboxDeltaFieldDefinition_t blackboxMainFields[] =
{
    ...
    {"loop",       -1, UNSIGNED, .Ipredict = PREDICT(0),       .Iencode = ENCODING(UNSIGNED_VB), .Ppredict = PREDICT(INC),           .Pencode = ENCODING_NULL,        CONDITION(ALWAYS)},
    ...
    {"command",     0, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_4S16),  CONDITION(COMMAND)},
    {"command",     1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_4S16),  CONDITION(COMMAND)},
    {"command",     2, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_4S16),  CONDITION(COMMAND)},
    {"command",     3, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_4S16),  CONDITION(COMMAND)},
    ...
    {"axisD",       0, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB),  CONDITION(PID)},
    {"axisD",       1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB),  CONDITION(PID)},
    {"axisD",       2, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB),  CONDITION(PID)},
    ...
    {"gyro",       0, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(GYRO)},
    {"gyro",       1, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(GYRO)},
    {"gyro",       2, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(GYRO)},

    {"accel",      0, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(ACC)},
    {"accel",      1, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(ACC)},
    {"accel",      2, SIGNED,    .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB),  CONDITION(ACC)},

#ifdef USE_MAG
    {"mag",         0, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MAG)},
    {"mag",         1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MAG)},
    {"mag",         2, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MAG)},
#endif
#ifdef USE_BARO
    ...
    {"rpm",         1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MOTOR_2)},
    {"rpm",         2, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MOTOR_3)},
    {"rpm",         3, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB),  CONDITION(MOTOR_4)},
...
static bool testBlackboxConditionUncached(FlightLogFieldCondition condition)
{
    ...
    case CONDITION(RPM_1):
        return (getMotorCount() > 0) && isFieldEnabled(FIELD_SELECT(RPM));
    case CONDITION(RPM_2):
        return (getMotorCount() > 1) && isFieldEnabled(FIELD_SELECT(RPM));
    case CONDITION(RPM_3):
        return (getMotorCount() > 2) && isFieldEnabled(FIELD_SELECT(RPM));
    case CONDITION(RPM_4):
        return (getMotorCount() > 3) && isFieldEnabled(FIELD_SELECT(RPM));

    case CONDITION(MOTOR_1):
        return (getMotorCount() > 0) && isFieldEnabled(FIELD_SELECT(MOTOR));
    ...
    case CONDITION(SERVO_1):
        return (getServoCount() > 0) && isFieldEnabled(FIELD_SELECT(SERVO));
...
static void writeIntraframe(void)
{
    ...
    if (testBlackboxCondition(CONDITION(MAG))) {
        blackboxWriteSigned16VBArray(blackboxCurrent->mag, XYZ_AXIS_COUNT);
    }
#endif
    ...
    if (isFieldEnabled(FIELD_SELECT(RPM))) {
        blackboxWriteSignedVBArray(blackboxCurrent->rpm, getMotorCount());
    }
...
#define CALC_DELTA(delta, next, prev, count) do {   \
    for (int i = 0; i < (count); i++)               \
...
static void writeInterframe(void)
{
     ...
     */
    if (testBlackboxCondition(CONDITION(COMMAND))) {
        CALC_DELTA(deltas, blackboxCurrent->command, blackboxLast->command, 4);
        blackboxWriteTag8_4S16(deltas);
    }

    if (testBlackboxCondition(CONDITION(SETPOINT))) {
        CALC_DELTA(deltas, blackboxCurrent->setpoint, blackboxLast->setpoint, 4);
        blackboxWriteTag8_4S16(deltas);
    }

    if (testBlackboxCondition(CONDITION(PID))) {
        CALC_DELTA(deltas, blackboxCurrent->axisPID_P, blackboxLast->axisPID_P, XYZ_AXIS_COUNT);
        blackboxWriteTag2_3S32(deltas);

        CALC_DELTA(deltas, blackboxCurrent->axisPID_I, blackboxLast->axisPID_I, XYZ_AXIS_COUNT);
        blackboxWriteTag2_3S32(deltas);

        CALC_DELTA(deltas, blackboxCurrent->axisPID_D, blackboxLast->axisPID_D, XYZ_AXIS_COUNT);
        blackboxWriteSignedVBArray(deltas, XYZ_AXIS_COUNT);

        CALC_DELTA(deltas, blackboxCurrent->axisPID_F, blackboxLast->axisPID_F, XYZ_AXIS_COUNT);
        blackboxWriteSignedVBArray(deltas, XYZ_AXIS_COUNT);
    }
    ...
    if (testBlackboxCondition(CONDITION(MAG))) {
        for (int i = 0; i < XYZ_AXIS_COUNT; i++) {
            deltas[packedFieldCount++] = blackboxCurrent->mag[i] - blackboxLast->mag[i];
        }
    }
#endif
#ifdef USE_BARO
    if (testBlackboxCondition(CONDITION(BARO))) {
        deltas[packedFieldCount++] = blackboxCurrent->baro - blackboxLast->baro;
    }
#endif

    if (testBlackboxCondition(CONDITION(VOLTAGE))) {
        deltas[packedFieldCount++] = (int32_t) blackboxCurrent->voltage - blackboxLast->voltage;
    }
    if (testBlackboxCondition(CONDITION(CURRENT))) {
        deltas[packedFieldCount++] = (int32_t) blackboxCurrent->current - blackboxLast->current;
    }
    if (testBlackboxCondition(CONDITION(RSSI))) {
        deltas[packedFieldCount++] = (int32_t) blackboxCurrent->rssi - blackboxLast->rssi;
    }
    ...
    if (isFieldEnabled(FIELD_SELECT(RPM))) {
        CALC_DELTA(deltas, blackboxCurrent->rpm, blackboxLast->rpm, motorCount);
        blackboxWriteTag8_8SVB(deltas, motorCount);
    }
    ...
    if (isFieldEnabled(FIELD_SELECT(SERVO))) {
        blackboxWriteMainStateArrayUsingAveragePredictor(offsetof(blackboxMainState_t, servo), getServoCount());
    }

    if (testBlackboxCondition(CONDITION(DEBUG))) {
        CALC_DELTA(deltas, blackboxCurrent->debug, blackboxLast->debug, DEBUG_VALUE_COUNT);
