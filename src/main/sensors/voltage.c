...
void voltageMeterADCInit(void)
{
    for (uint8_t i = 0; i < MAX_VOLTAGE_SENSOR_ADC && i < ARRAYLEN(voltageMeterAdcChannelMap); i++) {
        ...
        pt1FilterInit(&state->displayFilter, GET_BATTERY_LPF_FREQUENCY(batteryConfig()->vbatDisplayLpfPeriod), batteryConfig()->vbatUpdateHz);
    }
}
...
void voltageMeterESCInit(void)
{
    ...
    pt1FilterInit(&voltageMeterESCState.displayFilter, GET_BATTERY_LPF_FREQUENCY(batteryConfig()->vbatDisplayLpfPeriod), batteryConfig()->vbatUpdateHz);
#endif
}
