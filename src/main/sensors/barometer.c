...
static uint16_t calibCycles = 0;      // baro calibration = get new ground pressure value
...
void performBaroCalibrationCycle(void)
{
    ...
    float delta = ((float)baroPressure - baroGroundPressure) / ++currentCycle;

    if (fabsf(delta) < 1.0f || currentCycle > calibCycles) {
    ...
    switch (state) {
        ...
        case BARO_STATE_PRESSURE_SAMPLE:
            ...
            if (baroIsCalibrationComplete()) {
                baro.BaroAlt = lrintf(pressureToAltitude(baroPressure) - baroGroundAltitude);
            }
            else {
            ...
            DEBUG_SET(DEBUG_BARO, 3, baro.BaroAlt);
