...
#define ESCHW4_DIFFAMP_GAIN     13.6f
#define ESCHW4_DIFFAMP_SHUNT    (0.25f / 1000.0f)
#define ESCHW4_ADC_RESOLUTION   4096.0f
...
{
    return ESCHW4_V_REF * voltRaw / ESCHW4_ADC_RESOLUTION * escSensorConfig()->hw4_voltage_div;
}
...
static float calcCurrHW(uint16_t currentRaw)
{
    if (currentRaw > escSensorConfig()->hw4_current_offset) {
        return (currentRaw - escSensorConfig()->hw4_current_offset) *
            ...
            (ESCHW4_V_REF / (ESCHW4_DIFFAMP_GAIN * ESCHW4_DIFFAMP_SHUNT * ESCHW4_ADC_RESOLUTION));
    }
...
static bool processHW4TelemetryStream(uint8_t dataByte)
{
    if (skipBytes > 0) {
        ...
        // That's 84,650 seconds at 120Hz tranmission rate.  Not very likely to occur accidentally.
        bytesRead = 0;
        skipBytes = 11;
    }
    else if (bytesRead > 0) {
...
static void hw4SensorProcess(timeUs_t currentTimeUs)
{
    ...
    while (serialRxBytesWaiting(escSensorPort)) {
        if (processHW4TelemetryStream(serialRead(escSensorPort))) {
            ...
            //uint16_t thr = hwData[4] << 8 | hwData[5]; // 0-1024
            //uint16_t pwm = hwData[6] << 8 | hwData[7]; // 0-1024
            ...
            DEBUG(ESC_SENSOR, DEBUG_ESC_VOLTAGE, lrintf(voltage));
            DEBUG(ESC_SENSOR, DEBUG_ESC_CURRENT, lrintf(current));
