...
static uint8_t decodeTelemetryFrame(void)
{
    ...
    if (chksum == tlmsum) {
        ...
        if (currentEsc < 2) {
            DEBUG(ESC_SENSOR, DEBUG_ESC_RPM + currentEsc, erpm);
            DEBUG(ESC_SENSOR, DEBUG_ESC_TEMP + currentEsc, temp);
            DEBUG(ESC_SENSOR, DEBUG_ESC_VOLTAGE + currentEsc, volt);
            DEBUG(ESC_SENSOR, DEBUG_ESC_CURRENT + currentEsc, curr);
        }
