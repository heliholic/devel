...
#define EMFAT_MAX_ENTRY     (EMFAT_MAX_LOG_ENTRY + EMFAT_PREDEF_ENTRY_COUNT)
...
static const emfat_entry_t entriesPredefined[] =
{
    ...
    [EMFAT_ENTRY_PADDING]   = { "PADDING.TXT",  false, ATTR_HIDDEN, 1,  0,      0,               0,              0,                  CMA,  NULL,              NULL, { 0 } },
};
...
static char log_prefix[MAX_NAME_LENGTH + 1];
...
static void emfat_set_log_file_name(emfat_entry_t *entry, int number)
{
    ...
    if (entry->cma_time[0] == cmaTime) {
        ...
        tfp_sprintf(entry->name, "%s_%s.bbl", log_prefix, logNumber);
    } else {
        // Recognized timestamp, create a meaningful filename.
        tfp_sprintf(entry->name, "%s_%s_%04d%02d%02d_%02d%02d%02d.bbl",
                    log_prefix,
                    logNumber,
...
static void emfat_add_log(emfat_entry_t *entry, uint32_t offset, uint32_t size)
{
    entry->level = 1;
    entry->offset = offset;
...
void emfat_init_files(void)
{
...
#ifdef USE_FLASHFS
    strcpy(log_prefix, pilotConfig()->name[0] ? pilotConfig()->name : "RTFL");
    legalize_filename(log_prefix);

    flashInit(flashConfig());
    flashfsInit();
    LED0_OFF;

    int flashfsUsedSpace = flashfsGetOffset();
    ...
    if (logCount > 0) {
        ...
        emfat_add_log(entry, 0, flashfsUsedSpace);
    ...
    const int padding = FILESYSTEM_SIZE_MB * 1024 * 1024 - flashfsUsedSpace * 2;

    if (padding > 0) {
        memcpy(entry, &entriesPredefined[EMFAT_ENTRY_PADDING], sizeof(emfat_entry_t));

        emfat_set_entry_cma(entry);
        emfat_add_log(entry, 0, padding);
