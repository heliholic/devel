...
void sendRcDataToHid(void)
{
    ...
    for (unsigned i = 0; i < USB_CDC_HID_NUM_AXES; i++) {
        ...
        report[i] = scaleLimit(rcData[channel], PWM_RANGE_MIN, PWM_RANGE_MAX, USB_CDC_HID_RANGE_MIN, USB_CDC_HID_RANGE_MAX);
        if (channel == PITCH) {
    ...
    for (unsigned i = 0; i < USB_CDC_HID_NUM_BUTTONS; i++) {
        ...
        if (scaleLimit(rcData[channel], PWM_RANGE_MIN, PWM_RANGE_MAX, USB_CDC_HID_RANGE_MIN, USB_CDC_HID_RANGE_MAX) > 0) {
            report[8] |= (1 << i);
        }
    }
#if defined(STM32F4)
