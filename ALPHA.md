...
The servo code has been refactored, and is now working differently.
The CLI servo setup is reflecting the internal changes.

Please note that the Configurator will eventually have a more user friendly setup.
The CLI is not supposed to be used for anything else than initial alpha testing
and debugging. It is _not_ supposed to be easy to use...

The servo CLI command has the following new syntax:

```
servo <INDEX> <CENTER> <MIN> <MAX> <SCALE-> <SCALE+> <RATE> <FLAGS>

INDEX     Servo number 1..
CENTER    PWM pulse at the servo mid (center) point, where the servo arm is leveled
MIN       Minimum PWM pulse length
MAX       Maximum PWM pulse length
SCALE-    Scaling factor below mid-point
SCALE+    Scaling factor above mid-point
RATE      Servo update rate in Hz
FLAGS     Binary flags
```

NOTE! Each servo has an update rate parameter. If multiple servos are sharing the same
timer, the lowest value will be used among them.


### Servo Flags

The servo flags has currently two bit values:

```
 1   Servo is reversed
 2   Geometry correction enabled
```
...
Filter cutoff for altitude data (0.01Hz steps)

#### `rescue_var_cutoff = <N>`

Filter cutoff for vario data (0.01Hz steps)
