...
The CLI `servo` command has now different syntax to reflect the changes.

```
servo <center> <min> <max> <neg-scale> <pos-scale> <update-rate> <flags>

<center>       zero position in us (leveled servo arm position)
<min>          minimum pulse length
<max>          maximum pulse length
<neg-scale>    scaling factor on negative side (below level)
<pos-scale>    scaling factor on positive side (above level)
...
The servo flags is a binary flag value.
It has the following flags defined.
