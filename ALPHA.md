...
```
OFF    = No logging
BOX    = logging when BLACKBOX mode active
ARM    = logging when ARMED
ARMBOX = logging when both ARMED and BLACKBOX mode active
```

In BOX and ARM mode, a new log is created every time logging is enabled.
In ARMBOX, logging is paused if ARMed, but BLACKBOX mode is inactive.
